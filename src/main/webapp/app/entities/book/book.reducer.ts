...
export const getEntitiesByAuthor: ICrudGetAllByAction<IBook> = (id, page, size, sort) => {
  const requestUrl = `${apiUrl}/author/${id}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_BOOK_LIST,
    payload: axios.get<IBook>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
...
export const getEntitiesByPublisher: ICrudGetAllByAction<IBook> = (id, page, size, sort) => {
  const requestUrl = `${apiUrl}/publisher/${id}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_BOOK_LIST,
    payload: axios.get<IBook>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
